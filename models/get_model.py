...
from models.ppgn.ppgn import PPGN
...
def get_encoder(args, hidden):
    ...
    if args.dataset.lower() in ['zinc', 'alchemy', 'edge_wt_region_boundary', 'qm9', 'qm9_pos',
...
def get_model(args, *_args):
    ...
    num_candidates = (  # 2 if ensemble, 1 otherwise (also when only downstream)
        2 if hasattr(args, 'sample_configs') and
             args.sample_configs.separate and
             args.sample_configs.sample_k > 0 and
             args.sample_configs.sample_k2 > 0
        else 1
    )

    assert hasattr(args, 'downstream_model') and args.downstream_model in ['GNN', 'PPGN'], "Unsupported downstream model"

    if args.downstream_model == 'GNN':
    ...
    elif args.downstream_model == 'PPGN':
        model = PPGN(use_new_suffix=args.ppgn_configs.use_new_suffix,
