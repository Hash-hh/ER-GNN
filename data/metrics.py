...
def eval_mae(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    # Compute MAE along columns, ignoring NaNs in `y_true`
    mae_per_column = np.nanmean(np.abs(y_true - y_pred), axis=0)

    # Filter out columns that are entirely NaN
    mae_per_column = mae_per_column[~np.isnan(mae_per_column)]

    # Return the average MAE across columns
    return mae_per_column.mean() if mae_per_column.size > 0 else float('nan')
