...
def get_additional_path(args: Union[Namespace, ConfigDict]):
    ...
    if args.only_downstream:  # no need for candidate edges or pretransforms if only downstream
        return None
...
def get_pretransform(args: Union[Namespace, ConfigDict], pretransforms: Optional[List] = None):
    ...
    # add edge candidates
    if hasattr(args, 'sample_configs'):
        heu = args.sample_configs.heuristic if hasattr(args.sample_configs, 'heuristic') else 'longest_path'
        directed = args.sample_configs.directed if hasattr(args.sample_configs, 'directed') else False
...
def get_data(args: Union[Namespace, ConfigDict], *_args):
    ...
    elif args.dataset.lower() == 'qm9_pos':
        train_set, val_set, test_set, std = get_qm9_pos(args)  # also compatible with PPGN
    ...
    if args.downstream_model == 'PPGN':  # we need to group the data by size for PPGN
        dataloader = partial(create_same_size_dataloader,
...
def get_qm9_pos(args: Union[Namespace, ConfigDict]):
    pre_transform = get_pretransform(args, pretransforms=[GraphCoalesce()])

    data_path = os.path.join(args.data_path, 'QM9_pos_small') if args.debug else os.path.join(args.data_path, 'QM9_pos')  # small for debug
    extra_path = get_additional_path(args)
    if extra_path is not None:
        data_path = os.path.join(data_path, extra_path)
    ...
    for split in ['train', 'valid', 'test']:
        ...
        if args.downstream_model == 'PPGN':
            dataset = SizeGroupedQM9(data_path,
                          split=split,
                          transform=None,
                          pre_transform=pre_transform,
                          force_process=False,
    ...
    if args.debug and args.downstream_model != 'PPGN':  # PPGN does not support slicing
        train_set = train_set[:16]
        val_set = val_set[:16]
        test_set = test_set[:16]

    norm_const = [1] * 13  # Not normalizing here. Just for the sake of consistency
    std = 1. / torch.tensor(norm_const, dtype=torch.float)
